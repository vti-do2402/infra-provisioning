  ...
  instance_config = {
    ...
    bastion = {
      ...
      user_data = templatefile("${path.module}/user-data/bastion.sh", {
        ...
        KUBECTL_RELEASE_DATE = var.bastion.kubectl_release_date
        ARCH                 = var.bastion.arch
      })
      tags = {}
    }
    // MongoDB host
    mongodb = {
      ...
      user_data = templatefile("${path.module}/user-data/mongodb.sh", {
...
module "ec2_instances" {
  ...
  # iam_instance_profile        = data.aws_iam_instance_profiles.current[0].name
